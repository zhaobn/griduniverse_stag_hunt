...
def get_positions(file_name, game_id):
  ...
  # remove duplicates
  state_data_filtered['group_key'] = state_data_filtered['details'].str.extract(r'^(.*?"rows": 15, "items": \[{"id": \d+)', expand=False)
  state_data_filtered['creation_time'] = pd.to_datetime(state_data_filtered['creation_time'])
  state_data_filtered = state_data_filtered.sort_values('creation_time').groupby('group_key').tail(1).drop(columns=['group_key']).reset_index(drop=True)
...
def get_distance(file_name, game_id):
  ...
  required_items = ['hare1', 'hare2', 'stag1', 'player1', 'player2']
  ...
  for round_id in round_ids:
    ...
    if all(item in round_data['item_id'].tolist() for item in required_items):
      ...
      for player_id in ['player1', 'player2']:
        ...
        to_hare1 = manhattan_distance(player_pos[0], player_pos[1], hare1_pos[0], hare1_pos[1])
        to_hare2 = manhattan_distance(player_pos[0], player_pos[1], hare2_pos[0], hare2_pos[1])
        to_stag1 = manhattan_distance(player_pos[0], player_pos[1], stag1_pos[0], stag1_pos[1])
